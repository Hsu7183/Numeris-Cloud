(() => {
  ...
  async function loadDraws() {
    try {
      const response = await fetch("/api/draws?game_code=TW_DAILY539&date_from=2026-08-29&date_to=2026-09-21&page_size=50");
      if (!response.ok) throw new Error("無法讀取開獎資料");
      const draws = ((await response.json()).items || []).slice(0, 12).reverse();
